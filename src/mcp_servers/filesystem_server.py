...
from src.constants import NOTES_BASE
...
def list_study_files() -> list[str]:
    ...
    if not NOTES_BASE.exists():
        logging.info("Notes Base path directory dont exixits.")        
        return []
    
    logging.info(f"Returning all the markdown file from {NOTES_BASE}.")
    return sorted(
...
def read_study_file(filename: str) -> str:
    ...
    and  expose our API keys. We resolve both paths and verify the requested file 

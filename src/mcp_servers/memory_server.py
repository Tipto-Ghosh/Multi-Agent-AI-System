"""
The memory server gives agents a session-scoped key-value store. 
The Explainer writes which topics it has explained. The Progress 
Coach reads that history before deciding what to do next.
"""
...
""" 
In-progress store: {session_id: {key: {"value": str, "updated_at": str}}}

# TODO Replace with PostgreSQL. The MCP interface stays identical. Only this dict changes.
"""
...
def memory_get(session_id: str, key: str) -> str:
    """ 
    Retrive a value from session memory.
    
    Returns the stored value, or the string "null" if the key
    doesn't exist. Returns "null" (not Python None) so the LLM can handle the missing case
    without type errors.
    """
